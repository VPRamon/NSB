...
use nsb_data_tools::gaia::xp::bulk_index::{
...
/// Run the `index_gaia_xp_continuous_bulk` command using process arguments.
pub fn run_cli() -> Result<()> {
    ...
    let json_path = args.output_dir.join("xp_continuous_bulk_file_index.json");
    fs::write(&json_path, serde_json::to_string_pretty(&index)? + "\n")?;
    write_index_csv(
        &args.output_dir.join("xp_continuous_bulk_file_index.csv"),
        &index,
    )?;
