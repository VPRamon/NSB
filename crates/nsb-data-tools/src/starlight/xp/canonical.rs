...
fn parse_gaia_tuple_array(
    ...
    let body = trimmed
        ...
        .with_context(|| {
            format!(
                "{field_name} for source {} in {} is not a Gaia array",
                source_id.map_or_else(|| "unknown".to_string(), |value| value.to_string()),
                origin_file
    ...
    body.split(',')
        .map(|value| {
            let parsed = value
                .trim()
                .parse::<f64>()
                .with_context(|| format!("invalid numeric value in {field_name}"))?;
            if !parsed.is_finite() {
                bail!("non-finite numeric value in {field_name}");
            }
            Ok(parsed)
        })
        .collect()
