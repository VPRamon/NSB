...
use super::{
    integrate_photon_flux, integrate_photon_flux_uncertainty, CanonicalXpContinuousRecord,
    XpProduct, BAND_MAX_NM, BAND_MIN_NM, PINNED_GAIA_XPY_VERSION, XP_SAMPLED_GRID_STEP_NM,
};
...
impl GaiaXpContinuousCalibrator {
    ...
    /// Resolve an explicit, environment-adjacent, or repository-bundled design fixture.
    ...
    pub fn calibrate_record(
        ...
        let product = self.calibrate(record)?;
        let flux = integrate_photon_flux(&product)?;
        let uncertainty = integrate_photon_flux_uncertainty(&product)?;
    ...
    pub fn calibrate(&self, record: &CanonicalXpContinuousRecord) -> Result<XpProduct> {
        record.validate()?;
    ...
    /// Compatibility alias retained for the frozen parity oracle.
    pub fn calibrate_record_product(
        &self,
        record: &CanonicalXpContinuousRecord,
    ) -> Result<XpProduct> {
        self.calibrate(record)
    }
...
/// Integrate flux uncertainties with the GaiaXPy continuous-calibration trapezoid.
///
/// This matches the oracle fixtures used by the production bulk loop; do not replace
/// it with the sampled-XP `integrate_photon_flux` uncertainty path.
pub fn integrate_gaiaxpy_manifest_uncertainty(
    ...
    for index in 0..wavelengths_nm.len() - 1 {
        let dx = (wavelengths_nm[index + 1] - wavelengths_nm[index]).abs();
        integral += 0.5 * (photon_unc[index] + photon_unc[index + 1]) * dx;
    }
    Ok(integral)
}
