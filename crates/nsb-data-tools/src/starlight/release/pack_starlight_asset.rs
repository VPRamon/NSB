...
/// Run the `pack_starlight_asset` command using process arguments.
...
fn run(args: Args) -> Result<()> {
    ...
    if args.candidate && !validation_summary.production_ready {
        log::info!("warning: packing a non-production-ready starlight asset candidate");
    }
    ...
    {
        log::info!("warning: packing candidate with proxy or experimental photometry model");
        None
    } else {
        None
    };
...
fn ensure_integrated_runtime_header_defaults(header: &mut BTreeMap<String, String>) {
    ...
    set_default(
        header,
        "photometry_model",
        nsb_data_tools::starlight::integrated::INTEGRATED_PHOTOMETRY_MODEL,
    );
...
mod tests {
    ...
    #[test]
    fn production_rejects_incomplete_nside_attestation() -> Result<()> {
        let (args, _dir) = fixture_args(production_validation(), true)?;
        let review = args
            .approval_root
            .as_ref()
...
"#,
        ...
        let error = run(args).expect_err("incomplete nside review must fail closed");
...
"#
  ...
  "integrated_flux_conservation_pass": true
    ...
    fn file_sha256(path: &Path) -> Result<String> {
        Ok(format!(
            "sha256:{}",
            nsb_data_tools::platform::checksum_io::sha256_file(path)?
        ))
    }
}
