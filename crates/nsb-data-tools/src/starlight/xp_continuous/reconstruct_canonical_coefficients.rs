...
use nsb_data_tools::gaia::xp::continuous::{parse_datalink_gaiaxpy_csv, PHOTOMETRY_MODEL};
...
use nsb_data_tools::platform::checksum_io::sha256_file;
use std::fs;
...
/// Run XP continuous reconstruction with hierarchical command arguments.
pub fn run_cli() -> Result<()> {
    ...
    let manifest = serde_json::json!({
        ...
        "integration": {
            "implementation": "nsb-data-tools::gaia::xp::sampled::integrate_photon_flux",
            "band_nm": [336.0, 650.0],
            "grid_step_nm": 2.0,
...
mod tests {
    ...
    #[test]
    fn accepts_only_canonical_numeric_source_filenames() {
        assert_eq!(source_id_from_path(Path::new("456.csv")).unwrap(), "456");
        assert!(source_id_from_path(Path::new("xp_source_123.csv")).is_err());
    }
