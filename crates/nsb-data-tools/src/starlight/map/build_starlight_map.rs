...
/// Run the `build_starlight_map` command using process arguments.
...
fn verify_catalog_checksum(args: &Args) -> Result<()> {
    ...
    nsb_data_tools::platform::checksum_io::verify_sha256_file(&args.input, expected, "catalogue")
}
...
fn handle_science_diagnostic<E: std::fmt::Display>(
    ...
    match result {
        ...
        Err(err) => {
            log::info!("warning: starlight diagnostic {name} did not pass: {err}");
            Ok(())
        }
    }
}
...
mod tests {
    ...
    fn streaming_catalogue_checksum_matches_sha256sum() -> Result<()> {
        ...
        let expected = nsb_data_tools::platform::checksum_io::sha256_file(&input)?;
        let args = Args {
            input: input.clone(),
