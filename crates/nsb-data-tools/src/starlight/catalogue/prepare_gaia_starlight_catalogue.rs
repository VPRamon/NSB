...
use nsb_data_tools::gaia::xp::sampled::{
    integrate_sampled_photon_flux, parse_gaia_sampled_array_into, PhotonFluxIntegral, XpProduct,
...
/// Run the `prepare_gaia_starlight_catalogue` command using process arguments.
...
fn convert_normalized_row(
    ...
    let product = nsb_data_tools::gaia::xp::sampled::parse_normalized_record(headers, row)?;
...
fn convert_source(raw: GaiaDr3RawSourceRow, product: XpProduct, args: &Args) -> Result<Conversion> {
    ...
    let integral = nsb_data_tools::gaia::xp::sampled::integrate_photon_flux(&product)?;
    validate_integral(integral)?;
    if integral.total_ph_m2_s <= 0.0 {
        return Ok(Conversion::ScientificallyExcluded {
...
impl BulkColumns {
    fn from_headers(headers: &StringRecord) -> Result<Self> {
        if optional_header(headers, "wavelength").is_some() {
            bail!(
                "unsupported long-schema bulk file with per-row wavelength column; official Gaia XP sampled bulk stores flux arrays on an implicit 336–1020 nm grid"
            );
        }
...
mod tests {
    ...
    #[test]
    fn unsupported_long_schema_bulk_is_rejected() -> Result<()> {
        let dir = tempfile::tempdir()?;
        let bulk = dir.path().join("bulk");
        std::fs::create_dir(&bulk)?;
        write_gzip(
        ...
        assert!(error.to_string().contains("unsupported long-schema"));
