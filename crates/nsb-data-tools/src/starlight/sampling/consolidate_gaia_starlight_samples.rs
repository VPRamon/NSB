...
use nsb_data_tools::starlight::sampling::{
...
use nsb_data_tools::starlight::science::{DataPartition, SpatialSplitSpec};
use serde::Serialize;
...
/// Run the `consolidate_gaia_starlight_samples` command using process arguments.
...
fn run(args: Args) -> Result<()> {
    ...
    let (sources, memberships, split_counts) =
        nsb_data_tools::starlight::sampling::consolidate_stratified_samples(
            &inventory,
            &results_root,
            &split,
        )?;
    ...
    let manifest_path = output_dir.join("sampling_inputs.manifest.json");
    fs::write(
        &manifest_path,
    ...
    write_sha256sum(
        &output_dir,
        &[
            output_dir.join("sampling_job_inventory.json"),
            output_dir.join("sampling_job_inventory.csv"),
            output_dir.join("sampling_sources.csv"),
            output_dir.join("sampling_memberships.csv"),
            output_dir.join("sampling_split_assignments.csv"),
            output_dir.join("sampling_coverage.json"),
            manifest_path,
        ],
    )?;

    println!(
        "sampling complete: {} unique sources, {} memberships, splits {:?}",
        sources.len(),
        memberships.len(),
        split_counts
    );
...
fn write_inventory(
    dir: &Path,
    inventory: &[nsb_data_tools::starlight::sampling::JobInventoryEntry],
) -> Result<()> {
    fs::write(
        dir.join("sampling_job_inventory.json"),
        format!("{}\n", serde_json::to_string_pretty(inventory)?),
    )?;
    let mut writer = WriterBuilder::new().from_path(dir.join("sampling_job_inventory.csv"))?;
    writer.write_record([
        "job_id",
        "population",
        "stratum",
...
fn write_sources_csv(dir: &Path, headers: &[String], records: &[csv::StringRecord]) -> Result<()> {
    let path = dir.join("sampling_sources.csv");
...
fn write_memberships_csv(dir: &Path, memberships: &[(u64, String, String)]) -> Result<()> {
    let path = dir.join("sampling_memberships.csv");
    let mut writer = WriterBuilder::new().from_path(&path)?;
...
fn write_split_assignments(dir: &Path, rows: &[(u64, u64, String)]) -> Result<()> {
    let path = dir.join("sampling_split_assignments.csv");
    let mut writer = WriterBuilder::new().from_path(&path)?;
...
fn write_coverage(dir: &Path, report: &CoverageReport) -> Result<()> {
    fs::write(
        dir.join("sampling_coverage.json"),
        format!("{}\n", serde_json::to_string_pretty(report)?),
    )?;
    let md = render_coverage_md(report);
    fs::write(dir.join("sampling_coverage.md"), md)?;
    Ok(())
}
