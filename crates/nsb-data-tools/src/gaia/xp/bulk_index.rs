    ...
    if let Some(local_path) = result.local_path.clone() {
        ...
        if result.downloaded {
            result.row_found = Some(
                crate::gaia::xp::canonical::find_bulk_source(path, &source_id.to_string())?
                    .is_some(),
            );
        }
