...
/// Write a normalized reconstructed spectrum CSV (`source_id`, wavelength, flux, error).
pub fn write_normalized_spectrum_csv(
    path: &Path,
    product: &crate::gaia::xp::sampled::XpProduct,
) -> Result<()> {
    let mut writer = csv::WriterBuilder::new().from_path(path).with_context(|| {
        format!(
            "failed to create normalized spectrum CSV {}",
            path.display()
        )
    })?;
    writer.write_record([
    ...
    for (index, wavelength) in product.wavelengths_nm.iter().enumerate() {
        let flux = product.flux_w_m2_nm[index];
        let err = product
            .flux_error_w_m2_nm
            .as_ref()
