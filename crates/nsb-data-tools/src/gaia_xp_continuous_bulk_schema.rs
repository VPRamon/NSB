...
mod tests {
    ...
    fn inspect_pilot_prefix_when_available() -> Result<()> {
        let path = PathBuf::from(
            "/home/valles/nsb-data/starlight-gaia-release/pilot-xp-continuous-bulk/bulk/XpContinuousMeanSpectrum_000000-003111.csv.gz",
        );
        if !path.is_file() {
            return Ok(());
        }
    ...
    fn two_pilot_prefixes_are_compatible_when_available() -> Result<()> {
        ...
        let right = PathBuf::from(
            "/home/valles/nsb-data/starlight-gaia-release/pilot-xp-continuous-bulk/bulk/XpContinuousMeanSpectrum_003112-005263.csv.gz",
        );
