...
| B/V S10 constants for airglow | Component constants. | Python comments say values are calculated from SkyCalc spectrum after conversion. | Weak-to-medium; derivation is not reproduced. | **No** as constants. | Keep local for parity; add a derivation script or cited source before relying on them scientifically. |
...
| Standard Galactic starlight map | `components::starlight` expects `data/starlight_galactic_map_v1.csv` when the standard model is enabled. | No production catalogue-derived map is bundled yet. | Not yet scientifically traceable. | **No** as core SideRust. | Keep the map format and provenance in NSB; generate from a real catalogue before enabling by default. |
| Fixed SkyCalc starlight component | Removed legacy path; the old fixed spectrum is no longer loaded by production Rust code. | Python `CalculateSL` parity. | Strong for historical parity, weak as complete science model. | **No**. | Keep only as historical context; do not reintroduce without explicit legacy model naming. |
...
- the standard Galactic starlight-map product and its generator,
...
| `data/starlight_galactic_map_v1.csv` | Source catalogue, version/date, license, magnitude limits, band conversion method, map resolution, checksum. |
