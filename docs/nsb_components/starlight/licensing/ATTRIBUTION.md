...
`starlight_nside128.csv`, `merge_report.json`, `starlight_manual_seed_v1.csv`,
...
pipeline (`crates/nsb-data-tools`). NSB source itself uses BSD-3-Clause (see
the repository [`README.md`](../../../../README.md#licensing)). The
dependency graph includes astronomy crates with additional licence
obligations, including AGPL-3.0-only dependencies (see the repository
[`LICENSE`](../../../../LICENSE) for the combined-binary text); distributors
of combined binaries must review and comply with the complete dependency
terms. The Gaia-derived candidate artifacts additionally retain the Gaia
attribution and licence terms above because they are derived from Gaia bulk
data; they are not independently re-licensed by virtue of being processed by
NSB.
