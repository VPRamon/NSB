...
|---|---|---|---|---|---|---|---|
| Gaia XP 336–650 nm band, sampled grid and identifiers | `gaia_xp.rs`; release/validation binaries | Rust semantic ownership | `nsb-data-tools::gaia_xp`; generated `contracts/gaia_xp_photon_integration_v1.json` is the secondary contract | Rust catalogue/reconstruction tools | Scientific drift | Rust constants remain the sole authority; the generated contract is checked mechanically | `scientific_contract` equality and corruption tests |
| Photon energy and signed trapezoidal integration | Rust `integrate_photon_flux*` | Single implementation | `gaia_xp::integrate_photon_flux` | All production sampled and continuous integrations | Scientific correctness | One Rust implementation serves every retained workflow | Rust integration tests |
| Uncertainty propagation | Rust weighted sample-error variance | Single scientific-policy implementation | `gaia_xp` | Reconstructed contribution validation | Incorrect uncertainty coverage | Frozen tolerances remain in the generated contract | Contract equality/corruption tests and existing Phase 5 frozen evidence |
| Planck constant and speed of light | Local typed Planck constant plus `qtty::velocity::C` | Typed physical-constant ownership | qtty owns `C`; `gaia_xp` owns the single justified local SI Planck constant pending an upstream qtty constant | Photon conversion | Numerical drift | No duplicate implementation remains | Source policy test rejects literals outside Rust authority |
...
| Pipeline ordering and sibling process orchestration | Historical Phase 5B binaries and retained Rust services | Workflow duplicate | Retained Rust library services and durable commands | Release maintainers | Different defaults/resume/success criteria | Historical wrappers/binaries removed; policy test forbids retained Rust from invoking sibling binaries through `cargo run` | `retained_data_tools_do_not_spawn_sibling_cargo_binaries` |
...
- Python and shell tools are prohibited. Rust is the only implementation and
  orchestration language in this repository.
